...
## 收尾记录：Rulings 与延后项（2026-09-25）

**交付**：T1–T4 + 收尾小改动共 6 笔提交（`603e389` 计划 → `2e076b9`），43+ 文件 `+5854/−645` 量级。
**结论**：三条临时通道 → 一条缝（通道① 15 处调用点撤除、② 三处 UI 补丁删除、③ `tableUpdates` 换领域事件总线）；用户可见缺陷 3 修好（远端改期重挂 = `ROADMAP` P2.5#1 关单、远端删除的幽灵响铃、待办恢复重挂）+ 1 修好（事件恢复重挂，用户裁定保留提醒行后）。

### 裁定清单（`裁定 → 判错的代价`；需长期留存者已入 `DECISIONS.md` ADR 与 `ROADMAP.md` Pending）

| 类 | 裁定 |
|---|---|
| 范围/工期 | 工期由"半天"修正为 2–3 开发日 → 排期错位；缝**盖不住跨进程写**（`bin/dayspark.dart`）→ 靠重算兜底；不做事件持久化/重放 → 极端崩溃下短暂不一致；执行器拦截不做机制 → 漏发只剩编译期 + CI 一道防线 |
| 机制取舍 | 显式 scope 而非 Zone/拦截器 → 用漏发风险换编译期强制；允许**物化回写** `reminder.triggerTime` + D12 锚点归属 → 判错则非物化行重复施加位移；棘轮用行内容哈希 → 等量置换也红；守卫只 Dart 一份 → 少一道更早失败，避免同契约双实现漂移 |
| 行为变更 | 无 dueDate 的活跃待办其提醒改判 cancel（与 SPEC 规则 4 一致）；全天事件改期现在也重排提醒（修 bug）；删 `rescheduleRemindersProvider`（说谎的静默 API）；**本地软删保留提醒行**（用户 2026-09-25 拍板，与待办侧及远端 tombstone 对称） |
| 验收/流程 | 手工 3 项抽查**记账豁免**（用户拍板，沿用 P2/P4 先例）；G4 = 25；新功能编号取 3.5/D（不动 3.4 矩阵）；守卫是**句法层**守卫——白名单内"漏 `tx.applied`"由 316 条行为测试兜，不是守卫职责 |

### 过程教训（可复用）

1. **简报也会错**：本阶段编排者简报错了 3 次（锚点自相矛盾 → P1；通道① 漏项；G4 算术 + 两处前提），实现者与审查者各纠正过一次 —— 简报必须允许被反驳。
2. **"守卫全绿"不能当"没写表"的证据**：白名单目录让守卫看不见那一层（终审以证据链倒挂指出）。
3. **改既有断言必须显式**：本阶段唯一改的一条（`events_provider_test.dart:215-216`）先请示后改，并写进报告与 diff。
4. **async 测试不得用固定次数等待**：曾一次不可复现 `+266 -1`，改条件等待 + 参数化期望条数后 19 次压测全绿。
5. **审查者自证也要防空转**：终审者自曝其 push 探针首版因 cast 错而"空转绿"，靠 DIAG 打印抓出。

### 延后项（b 级，已入 `ROADMAP.md` Pending）

ICS 导入接 outbox（P2.5#2）· 本地新建实体不挂默认提醒（产品决策）· CLI 跨进程写接入 · `reminders.triggerTime` 系派生态存表（改存 `offsetMinutes`，与 D1 一并收敛）· T2 残留面（行归属辨错 / 跨进程只改提醒行被缓存早退跳过）· 重排器串行链无专门测试 · `knownClean` 早退窄洞（可达性≈0）· 逃生门 `scheduleReminderProvider` 零调用者（留一版后删）。
